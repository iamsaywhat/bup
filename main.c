...
//	}
	...
	{
		// Если не вставлены,  гасим, Неисправность зажигаем
		// Гасим "Готовность"
		LED_READY_OFF();
	...
	PYRO_ON();
	// Пиропатрон включаем только на 3 сек - но тупо ждать нельзя -------переделать!
	delay_us(3000000);
	...
	while(1)
	{
		...
		{
			// Да, просит. Необходимо запросить информацию у СНС и СВС
      ...
      printf("Hello world!\n");
			...
			// Сбросим таймаут
			timeout = 0;
			// Пытаемся получить данные от СНС, не более 20 попыток
			while(!SNS_GetPositionData(&SNS_Position) && (timeout != 20)) timeout ++;
			// Если это был выход по таймауту, возможно связь с СНС пропала, нужно проверить
			if(timeout == 20)
			{
				// Узнаем состояние связи с СНС
				SelfTesting_SNS();
			}
			// Сбросим таймаут
			timeout = 0;
			...
			// Если это был выход по таймауту, возможно связь с СВС пропала, нужно проверить
			if(timeout == 20)
			{
				SelfTesting_SWS();
			}
