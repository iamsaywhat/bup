...
#include "actuator.h"                            // Драйвер работы с БИМ
...
//	}
  ...
  {
    ZPZ_init();                                      /* Запускаем драйвер ЗПЗ */
    BIM_Supply_ON();                                 /* Для тестирования БИМов подадим на них питание */
    while(!CONNECT_ZPZ_CHECK)                        /* И уходим на обслуживание режима ЗПЗ */
    {
      ZPZ_Service();                                 /* Запускаем модуль обслуживания ЗПЗ */
      if(ZPZ_CheckHighPriorityTask() == ZPZ_SC_MODE) /* Если ЗПЗ в настоящий момент не занят тяжелым процессом (режим РК) */
        TaskManagerZPZBackgroundRun ();              /* Будем в свободное время будем заниматься самодиагностикой */		
    }
  }
  ...
  BIM_Supply_ON();                  /* Включаем питание БИМов */
  ...
  BUP_DataUpdate ();                /* Запускаем обновление данных */
  TaskManagerGo2Task (TaskTest, 0); /* Переключаем планировщик на первую из пассивных задач (диагностика) */
	
  while(1)                          /* Главный рабочий цикл */
  {                                 /* Начинаем выполнять рабочий цикл, */
    TaskManagerRun();	              /* который перезапускается по таймеру */
  }
}
