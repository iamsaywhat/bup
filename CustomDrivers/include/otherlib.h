/***********************************************************************************************************
    ...
    В настоящий момент модуль осуществляет настройку системной шины HCLK на 80 МГЦ (при HSE = 8 МГц).
	  Функции настройки таймеров опираются на данную частоту.
		...
		* Clock_init           - Настройка тактирования процессора на 80 МГЦ (при HSE = 8 МГц);
		* SysTick_init         - Инициализация SysTick таймера на отсчет системного времени с дискретностью 1 мс;
		...
		* Timer_init           - Инициализация таймера на прерывания с заданной дискретностью
...
/**************************************************************************************************************
  Timer_init - Настройка таймера TIMERx на прерывание каждые ticks тактов.
  Параметры:
