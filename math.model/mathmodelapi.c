...
{	
    ...
    if (rtY.touchdown == 0)
      MathModel_sendBimCommand (rtY.leftStrap, rtY.rightStrap);
...
void MathModel_sendBimCommand (uint8_t left, uint8_t right)
{
  ...
  if(left > 0 && right == 0) // нужно затянуть левый БИМ в положене left и отпустить правый в 0
  {
    /* Этот каскад условий необходим, чтобы: 
    1) Запретить одновременную работу двигателей, принудительным переходом через ноль
    (если было: левый 50%, а сейчас пришла команда правый 50%, то сначала левый переводится в ноль,
  ...
  {
    if (BIM_getStrapPosition (RIGHT_BIM) != right)                   // Требуется затянуть левый БИМ, правый при этом должен быть в положении 0 
      BIM_sendRequest (RIGHT_BIM, BIM_CMD_ON, right, 9, 255, 255);   // Правый не в нуле, поэтому пока только переведём его в ноль
      
    if (BIM_getStrapPosition (RIGHT_BIM) != left)                    // Требуется затянуть левый БИМ, правый при этом должен быть в положении 0 
      BIM_sendRequest (RIGHT_BIM, BIM_CMD_ON, left, 9, 255, 255);    // Правый не в нуле, поэтому пока только переведём его в ноль
  }
